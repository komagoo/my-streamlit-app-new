...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        if os.path.exists(logo_path):
            ...
            st.markdown("<h1 style='text-align: center; color: #ff4b4b;'>🚀 HERO</h1>", unsafe_allow_html=True)
        ...
        with st.container(border=True):
            st.markdown("<h2 style='text-align: center; color: #333; margin-top: 0; margin-bottom: 20px;'>로그인</h2>", unsafe_allow_html=True)
            
            with st.expander("💡 테스트 계정 안내", expanded=False):
                st.markdown("""
                    <p style="margin: 5px 0;"><strong>관리자:</strong> admin123 / admin123!@#</p>
                    <p style="margin: 5px 0;"><strong>Etch:</strong> etch123 / etch123!@#</p>
                    <p style="margin: 5px 0;"><strong>Photo:</strong> photo123 / photo123!@#</p>
                    <p style="margin: 5px 0;"><strong>Diffusion:</strong> diff123 / diff123!@#</p>
                    <p style="margin: 5px 0;"><strong>Thin Film:</strong> thin123 / thin123!@#</p>
                    <p style="margin: 5px 0;"><strong>C&C:</strong> cc123 / cc123!@#</p>
                    <p style="margin: 5px 0;"><strong>수율:</strong> yield123 / yield123!@#</p>
                """, unsafe_allow_html=True)
            
            with st.form("login_form"):
                role_option = st.selectbox("👥 부서 선택", ["관리자", "Etch", "Photo", "Diffusion", "Thin Film", "C&C", "수율"], key="role_select_new")
        ...
        with col3:
            ...
            if need_cols_top5.issubset(df.columns):
                ...
                if not top5_actions.empty:
                    ...
                    st.info("조치별 작업시간 데이터가 없습니다.")
            else:
                st.info("시간 컬럼이 없어 TOP5를 계산할 수 없습니다.")

        # 우: 문제원인 치명도 TOP5
        with col4:
            st.markdown("##### 🔥 문제별 심각도 TOP5")
            _agg = (
            ...
            if not _score_top5.empty:
                ...
                st.info("문제별 심각도 데이터가 없습니다.")

        # AI 인사이트 - 조치별 작업시간과 문제별 심각도 통합
        if need_cols_top5.issubset(df.columns) and 'top5_actions' in locals() and not top5_actions.empty and not _score_top5.empty:
            with st.spinner("AI가 심화 분석을 진행하고 있습니다..."):
                ...
                _summ2 = [
                    f"{r['문제원인']}(점수 {float(r['치명도점수']):.2f}, 평균리드타임 {float(r.get('평균리드타임_h', 0) or 0):.2f}h)"
                    for _, r in _m.iterrows()
                ]
                
                prompt_combined = f"""
다음 데이터를 종합 분석하여 운영 효율화 관점에서 인사이트를 제공해주세요:

작업시간이 긴 조치 Top5: {'; '.join(_summ)}
치명도가 높은 문제 Top5: {'; '.join(_summ2)}

예방정비 우선순위와 리소스 배분 관점에서 3-4문장으로 요약해 주세요. 번호 매기기나 불릿은 사용하지 말고 자연스러운 문장으로 작성해주세요.
"""
                insight_combined = llm.predict(prompt_combined)
                st.markdown(
                    f"""
                    <div class="success-card">
                        <h4 style="margin-top: 0;">🔍 종합 운영 인사이트</h4>
                        <p style="margin-bottom: 0; line-height: 1.6;">{insight_combined}</p>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
